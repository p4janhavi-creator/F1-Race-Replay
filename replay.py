...
# --- DRAW THE TRACK AND LEADERBOARD SIDE BY SIDE ---
...
ver_pos = all_drivers["1"]
ax.plot(ver_pos["X"], ver_pos["Y"], color="gray", linewidth=2, zorder=0)
ax.set_aspect("equal")
ax.axis("off")
ax.set_title("Monaco 2023 - Replay", color="white", fontsize=16)
...
# Create dots and labels for track
dots = {}
labels = {}
...
# Create leaderboard text rows — one per position
leaderboard_rows = []
for i in range(20):
    row_text = ax_lb.text(
        0.05, 0.95 - (i * 0.047), "",
        ...
        fontfamily="monospace"  # monospace keeps columns aligned
    )
    leaderboard_rows.append(row_text)

# Get all laps for leaderboard lookups
...
def get_leaderboard(current_time):
    """For a given moment in time, return drivers sorted by position"""
    standings = []

    for driver_number, abbr in driver_map.items():
        # Get all laps for this driver up to current time
